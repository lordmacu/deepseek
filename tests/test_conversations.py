...
def test_a_first_message_opens_a_conversation(store):
    assert store.resolve([turn("user", "hola")]) is not None


def test_a_follow_up_continues_the_same_conversation(store):
    first = [turn("user", "hola")]
    cid = store.resolve(first)
    store.record(cid, first, "¡hola!")
...
def test_a_third_turn_still_continues(store):
    msgs = [turn("user", "uno")]
    cid = store.resolve(msgs)
    store.record(cid, msgs, "R1")
...
def test_an_unrelated_thread_gets_its_own_conversation(store):
    a = [turn("user", "hola")]
    cid_a = store.resolve(a)
    store.record(cid_a, a, "¡hola!")
    assert store.resolve([turn("user", "otra cosa")]) != cid_a
...
def test_two_clients_asking_the_same_first_question_are_not_one_conversation(store):
    ...
    assert store.resolve([turn("user", "hola")]) != store.resolve([turn("user", "hola")])
...
def test_the_title_comes_from_the_first_user_message(store):
    cid = store.resolve([turn("system", "sos útil"), turn("user", "qué es un tensor")])
...
def test_image_parts_are_not_stored_as_text(store):
    ...
    cid = store.resolve([turn("user", content)])
    store.record(cid, [turn("user", content)], "ok")
...
def test_listing_is_newest_first_and_paginates(store):
    ids = []
    for i in range(5):
        msgs = [turn("user", f"pregunta {i}")]
        cid = store.resolve(msgs)
        store.record(cid, msgs, f"respuesta {i}")
        ids.append(cid)
...
def test_a_known_but_empty_conversation_returns_an_empty_list(store):
    cid = store.resolve([turn("user", "hola")])
    assert store.messages_of(cid) == []
...
def test_without_a_database_nothing_raises(broken_store):
    ...
    broken_store.record(None, [turn("user", "hola")], "¡hola!")
