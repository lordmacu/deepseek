...
| `conversations` | ❌ | — | `server.py` no expone ninguna ruta `/v1/conversations*`. El endpoint upstream existe y está **confirmado en el APK decompilado de este repo**: `/api/v0/chat/history_messages`, en `decompiled_jadx/sources/defpackage/z66.java`. No aparece en el módulo `deepseek` ni en el documento de la spec — que en su §1 solo nombra, para deepseek, `/api/v0/file/upload_file` y un "RAG index" genérico. Exponerlo es trabajo pendiente; el booleano dice `false` porque hoy no hay ruta que lo sirva. |
