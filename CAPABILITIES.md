...
| `conversations` | ❌ | — | `server.py` no expone ninguna ruta `/v1/conversations*`. El encargo de esta tarea nombraba `/api/v0/chat/history_messages` como el endpoint upstream correspondiente, pero **esa ruta no se pudo confirmar acá**: no aparece en el módulo `deepseek`, en los binarios decompilados de este repo (`decompiled_apktool/`, `decompiled_jadx/`), ni en el documento de la spec -- que en su §1 solo nombra, para deepseek, `/api/v0/file/upload_file` y un "RAG index" genérico, sin mencionar `history_messages`. Se deja constancia del nombre reportado, sin atribuirlo a una fuente que no lo respalda; confirmarlo contra una cuenta real o el cliente decompilado queda pendiente para quien retome esta capacidad. |
